...
classNo = [] # array of classes
myList = os.listdir(path) # myList = list with all names of the DataTraffic folder
print("Total Classes Detected:", len(myList)) # there is 43 classes
noOfClasses = len(myList)
print("Importing Classes.....")
for x in range(0, len(myList)):
    myPicList = os.listdir(path + "/" + str(count)) # inside to the first folder, after second,...,
    for y in myPicList:
        curImg = cv2.imread(path + "/" + str(count) + "/" + y) # reading the first image of the first folder...
        images.append(curImg) # inside to array the images that returns from cv2.imread
        classNo.append(count) # classNo = [0,1,2,...,0,1,2,.....,0,1,2,......]
    print(count, end=" ")
    count += 1
print(" ")
...
#split the images for the test and train
X_train, X_validation, y_train, y_validation = train_test_split(X_train, y_train, test_size=validationRatio)
#split the images from the train to validation
...
def myModel():
    ...
    model.add(MaxPooling2D(pool_size=size_of_pool))  # DOES NOT EFECT THE DEPTH/NO OF FILTERS
...
history = model.fit(dataGen.flow(X_train, y_train, batch_size=batch_size_val), steps_per_epoch=steps_per_epoch_val,
                    epochs=epochs_val, validation_data=(X_validation, y_validation), callbacks=my_callbacks)
